...
static textparser_token_item *textparser_parse_token(textparser_handle *int_handle, const language_definition *definition, int token_id, int parent_end_token_id, size_t offset)
{
    ...
    switch(token_def->type)
    {
            ...
            do {
                ...
                if ((closest < SIZE_MAX)&&(closest < closest_parent_end))
                {
                    if (child == NULL) {
                        ...
                        child->next = textparser_parse_token(int_handle, definition, current_token_id, parent_end_token_id, offset);
                        child = child->next;
                    }
        ...
        case TEXTPARSER_TOKEN_TYPE_GROUP_ALL_CHILDREN_IN_SAME_ORDER:
            ...
            for (int c = 0; token_def->nested_tokens[c] != -1; c++)
            {
                ...
                if (c == 0) {
                    ...
                    child->next = textparser_parse_token(int_handle, definition, token_def->nested_tokens[c], parent_end_token_id, offset);
                    child = child->next;
                }
        ...
        case TEXTPARSER_TOKEN_TYPE_GROUP_ONE_CHILD_ONLY:
            ...
            break;
        case TEXTPARSER_TOKEN_TYPE_SIMPLE_TOKEN:
            if (!adv_regex_find_pattern(token_def->start_string, int_handle->text_format, int_handle->text_addr + current_offset, int_handle->text_size - current_offset, &token_start, &len))
            {
        ...
        case TEXTPARSER_TOKEN_TYPE_START_OPT_STOP:
            ...
            if ((!adv_regex_find_pattern(token_def->end_string, int_handle->text_format, int_handle->text_addr + current_offset, int_handle->text_size - current_offset, &token_end, &len))&&(token_def->type == TEXTPARSER_TOKEN_TYPE_START_STOP))
            {
...
EXPORT_TEXT_PARSER int textparser_parse(textparser_t handle, const language_definition *definition)
{
    ...
    while(pos < size) {
        ...
        textparser_token_item *token_item = textparser_parse_token(handle, definition, closest_token_id, -1, closest_offset);
