...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        printf("Usage: %s <asm_code_string>\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    res = textparser_parse(handle, &asm_definition);
    if (res != 0) {
    ...
    if (res != 0) {
        printf("Failed to register ASM validators. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_match_result match = {0};
    res = textparser_execute_language_grammar(handle, &asm_definition, &match);
    if (res != 0) {
        printf("Failed to execute ASM grammar. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_validation *validation = textparser_validate_asm(handle);
    if (validation != NULL) {
