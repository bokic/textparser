...
int main(int argc, char *argv[]) {
    ...
    res = textparser_parse(handle, &sql_definition);
    if (res != 0) {
    ...
    res = textparser_sql_register_validators(handle);
    if (res != 0) {
        printf("Failed to register SQL validators. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_match_result match = {0};
    res = textparser_execute_language_grammar(handle, &sql_definition, &match);
    if (res != 0) {
        printf("Failed to execute SQL grammar. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_validation *validation = textparser_validate_sql(handle);
