...
int main(int argc, char *argv[]) {
    ...
    res = textparser_parse(handle, &r_definition);
    if (res != 0) {
    ...
    if (res != 0) {
        printf("Failed to register R validators. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_match_result match = {0};
    res = textparser_execute_language_grammar(handle, &r_definition, &match);
    if (res != 0) {
        printf("Failed to execute R grammar. Error `%s`, code %d\n", textparser_parse_error(handle), res);
        textparser_close(handle);
        return EXIT_FAILURE;
    }

    textparser_validation *validation = textparser_validate_r(handle);
    if (validation != NULL) {
