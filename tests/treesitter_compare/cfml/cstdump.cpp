// Parse a file with the textparser CFML grammar and print the canonical
// CST JSON (same form as the TypeScript harness output). Mirrors
// tests/treesitter_compare/cstdump.cpp.
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr, "usage: %s <file.cfm|.cfc|.cfml>\n", argv[0]);
        return 1;
    }
    ...
    if (textparser_json_load_language_definition_from_json_file(
            "definitions/cfml_definition.json", &definition) != TEXTPARSER_JSON_NO_ERROR) {
        fprintf(stderr, "cannot load cfml_definition.json (run from repo root)\n");
        return 1;
    }
