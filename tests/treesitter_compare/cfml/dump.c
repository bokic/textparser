...
extern const TSLanguage *tree_sitter_cfquery(void);

static void usage(const char *prog) {
    fprintf(stderr,
            "usage: %s <file.cfm|.cfc|.cfs|.cfml> [--outline] [--positions]\n"
...
/* Grammar selection mirrors the zed-cfml language configs:
 *   .cfm/.cfc/.cfml -> cfml, .cfs -> cfscript, .cfq -> cfquery, otherwise -> cfml */
static const TSLanguage *pick_language(const char *path) {
    const char *dot = strrchr(path, '.');
    if (dot && (strcmp(dot, ".cfs") == 0)) return tree_sitter_cfscript();
    if (dot && (strcmp(dot, ".cfq") == 0)) return tree_sitter_cfquery();
    return tree_sitter_cfml();
}
...
int main(int argc, char **argv) {
    ...
    const TSLanguage *lang = pick_language(path);
