...
#ifdef Q_OS_WIN
#define KBD_LCTRL      29
#define KBD_RCTRL     285
#define KBD_HOME      327
#define KBD_END       335
#define KBD_UP        328
#define KBD_DOWN      336
#define KBD_LEFT      331
#define KBD_RIGHT     333
...
void QCodeEditWidget::keyPressEvent(QKeyEvent *event)
{
    ...
    {
        for(int c = 0; c < l_EventText.length(); c++)
        {
...
void QCodeEditWidget::paintEvent(QPaintEvent *event)
{
    	...
    	//if ()
    	{
    		...
    		if ((has_selection)&&(m_ScrollYLinePos + l_line >= (selection_start_row - 1))&&(m_ScrollYLinePos + l_line <= (selection_end_row - 1)))
    		{
    			if (selection_start_row == selection_end_row)
    			{
    				...
    				if (to > from)
                    {
                        ...
                        painter.fillRect(l_LineNumbersPanelWidth + (from * l_fontWidth), l_line * l_fontHeight, ((to - from) * l_fontWidth), l_fontHeight, style()->standardPalette().brush(QPalette::Highlight));
                    }
    			}
            ...
            if (!l_Line.isEmpty())
    		{
                ...
                if (colorItems.count() == 0)
                {
                    ...
                    painter.setPen(style()->standardPalette().color(QPalette::WindowText));
    ...
    if ((m_currentlyBlinkCursorShowen == 1)&&((int)m_CarretPosition.m_Column - (int)m_ScrollXCharPos - 1 >= 0))
    {
        ...
        painter.fillRect(xpos, ((m_CarretPosition.m_Row - m_ScrollYLinePos - 1) * l_fontHeight), ceil(m_LineHeight * 0.1), l_fontHeight - 1, style()->standardPalette().brush(QPalette::HighlightedText));
    }
...
void QCodeEditWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if ((event->button() == Qt::LeftButton)&&(event->position().x() < 16))
    {
        ...
        {
            emit breakpoint_change(l_LineHited);
        }
...
void QCodeEditWidget::setText(QString text)
{
    ...
    if ((m_Lines.isEmpty())||(m_Lines.last().type != 0))
    {
        l_Line.text = "";
        l_Line.type = 0;
        m_Lines.push_back(l_Line);
