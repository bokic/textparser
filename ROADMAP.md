...
> **Status**: The v2 compiler-grade parser engine (declarative EBNF runtime, Pratt precedence parsing, lexer modes, lexical goals, speculative checkpoints, structured trivia, AST lifecycle, and diagnostic recovery) is implemented in the C core runtime (`src/textparser.c`). TypeScript/TSX, PHP, CFML, JSON, and Bash use primary v2 definitions (`definitions/{typescript,php,cfml,json,bash}_definition.json`) with full EBNF grammars, stack-based lexer modes, and error recovery. The legacy scanners for CFML, JSON, and Bash are retained as `definitions/{cfml,json,bash}_legacy_definition.json` for the legacy tokenization tests and the core lexer suites.

### Remaining Work:
- **Migrate Remaining Languages to v2 Grammar Schema** (25 remaining; `typescript`, `php`, `cfml`, `json`, `bash` done):
  - Author real EBNF grammars, stack-based lexer modes (`pushMode`/`popMode`), contextual lexical goals, and Pratt operator precedence tables for other languages (ada, asm, c, c3, cpp, csharp, css, fortran, go, html, jai, java, javascript, matlab, md, pascal, perl, python, r, rust, scratch, sql, swift, vb, zig).
